 ...
 */
async function openNewRotationModal(
  bot,
  client,
  triggerId,
  channel,
  ...
  const existingConfig = preName
    ? bot.configStore.getItem(channel, preName)
    : null;
  const view = buildNewRotationView(
    channel,
    preName,
    existingConfig,
    bot.timezoneOptions,
...
async function buildRotationsViewBlocks(bot, channel) {
  ...
  blocks.push({
    ...
    elements: [
      {
        type: "button",
        text: { type: "plain_text", text: "Rotations" },
        action_id: "open_rotations_tab",
        style: "primary",
        value: channel,
      },
      {
        type: "button",
        text: { type: "plain_text", text: "Leave Calendar" },
        action_id: "open_leave_tab",
        value: channel,
      },
    ],
  });
  ...
  if (validRotations.length > 0) {
    ...
    blocks.push({
      ...
      elements: [
        {
          type: "mrkdwn",
          text: "_Queue order may change with skips and daily resets._",
        },
      ],
    });
    ...
    for (const name of validRotations) {
      ...
      if (members.length > 0) {
        try {
          ...
          blocks.push({
            type: "section",
            text: { type: "mrkdwn", text: "_Could not load queue._" },
          });
        }
      } else {
        blocks.push({
          type: "section",
          text: { type: "mrkdwn", text: "_This rotation has no members._" },
        });
      }
...
function buildQueueBlocks(bot, channel, name, cfg, members) {
  ...
  if (schedule.length === 0) {
    return [
      {
        type: "section",
        text: { type: "mrkdwn", text: "_No members in queue._" },
      },
    ];
  }
  ...
  const nextPickDates = estimateNextPickDates(
    schedule,
    cfg,
    bot.leaveStore,
    channel,
  ...
  const headerBlock = {
    ...
    fields: [
      ...
      { type: "mrkdwn", text: "*Next up (estimated)*" },
    ],
  };
...
module.exports = {
  openSelectModal,
  openNewRotationModal,
  buildNewRotationViewForBot,
  buildRotationsViewBlocks,
  buildRotationsView,
};
